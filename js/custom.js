...
//   $("a.no-link[href]").click(function () {
//     var self = $(this);
//     self.toggleClass("active").next().slideToggle(4000, function () {
...
(function ($) {
  ...
  var element_position = $('.count').offset().top;
  ...
  /*------------------------
      Image Interactivity (Tilt --> Mouse Position)
  -------------------------- */
  ...
  const profFollow = () => {
      ...
      $("html").mousemove(function (event) {
          var $prof = $(".profile");
          var $gloss = $('.semigloss');
          var $gloss2 = $('.semigloss2');
          var $x = ($prof.offset().left) + ($prof.width() / 2);
          var $y = ($prof.offset().top) + ($prof.height() / 2);
          var $mouseX = event.pageX - $x;
          var $mouseY = event.pageY - $y;
          //var $rot = ($rad * (180 / Math.PI) * -1) + 180;
          var $rotateX = (-1) * 10 * $mouseY / $prof.width();
          var $rotateY = (+1) * 10 * $mouseX / $prof.height();
          ...
          $gloss2.css({
              'transform': 'translate(' + -$mouseX / 6 + 'px)'
          });
      });
  }
  ...
  $('.grid_container').on('click', function() {
    $('.grid_container').toggleClass('visible');
  });
...
//   if (idleTime >= 5000) {
//     document.getElementById('alert-inactive').style.opacity = '1'
//     document.getElementById('alert-inactive').style.pointerEvents = 'all'
//   }
// }
